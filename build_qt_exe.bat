...
rmdir /s /q build\NemexiaRaidManagerQt 2>nul
del /q dist\NemexiaRaidManagerQt.exe 2>nul
"%VENV_PY%" -m PyInstaller NemexiaRaidManagerQt.spec --noconfirm
if errorlevel 1 goto :pyinstaller_error
call :message qt_build_success
if /i "%~1"=="--pause" pause
endlocal & exit /b 0
