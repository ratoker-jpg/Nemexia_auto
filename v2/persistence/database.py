...
class V2Database:
            ...
            with conn:
                conn.execute(
                    """INSERT INTO raid_actions(
                        request_id, source, target, player, ship_count,
                        status, created_at, updated_at
                    ) VALUES(?, ?, ?, ?, ?, 'pending', ?, ?)""",
                    (
                        str(request_id), str(source), str(target), str(player),
                        int(ship_count), now, now,
                    ),
                )
    ...
    def finish_raid_action(
        ...
        *,
        ...
        if status not in {"verified", "ambiguous", "rejected", "failed"}:
    ...
    def begin_spy_action(
        self, *, request_id: str, fleet_id: str, source: str, target: str
    ) -> None:
        conn = self._require_conn()
        now = datetime.now(timezone.utc).replace(microsecond=0).isoformat()
        try:
            with conn:
                conn.execute(
                    """INSERT INTO spy_actions(
                        request_id, fleet_id, source, target,
                        status, created_at, updated_at
                    ) VALUES(?, ?, ?, ?, 'pending', ?, ?)""",
                    (
                        str(request_id), str(fleet_id), str(source), str(target),
                        now, now,
                    ),
                )
        except sqlite3.IntegrityError as exc:
    ...
    def finish_spy_action(
        ...
        *,
        ...
        if status == "verified" and (
            not str(report_id or "").strip() or not str(report_at or "").strip()
        ):
        ...
        with conn:
            cursor = conn.execute(
                """UPDATE spy_actions
                   SET status=?, report_id=?, requested_at=?, report_at=?,
                       detail=?, updated_at=?
                 WHERE request_id=? AND status='pending'""",
                (
                    status, report_id, requested_at, report_at,
                    str(detail or ""), now, str(request_id),
                ),
            )
        if cursor.rowcount != 1:
        ...
        with conn:
            cursor = conn.execute(
                """UPDATE spy_actions
                 ...
                 WHERE request_id=? AND status IN ('pending','ambiguous')""",
                (
                    str(report_id), str(report_at), str(detail or ""),
                    now, str(request_id),
                ),
            )
        if cursor.rowcount != 1:
    ...
    def list_spy_actions(self, *, limit: int = 200) -> list[dict[str, object]]:
        rows = self._require_conn().execute(
            "SELECT * FROM spy_actions ORDER BY id DESC LIMIT ?",
            (max(1, int(limit)),),
        ).fetchall()
        return [dict(row) for row in rows]
    ...
    def unresolved_spy_action(self, *, fleet_id: str) -> dict[str, object] | None:
        row = self._require_conn().execute(
            """SELECT * FROM spy_actions
                WHERE fleet_id=? AND status IN ('pending','ambiguous')
                ORDER BY id DESC LIMIT 1""",
            (str(fleet_id),),
        ).fetchone()
        return dict(row) if row is not None else None

    def unresolved_spy_target(
        self, *, source: str, target: str
    ) -> dict[str, object] | None:
        row = self._require_conn().execute(
            """SELECT * FROM spy_actions
    ...
    def raid_queue_count(self) -> int:
        return int(
            self._require_conn().execute("SELECT COUNT(*) FROM raid_queue").fetchone()[0]
        )
    ...
    def import_raid_queue_rows(self, rows: Sequence[Mapping[str, object]]) -> int:
        ...
        payload = [
            (
                int(row["legacy_id"]), int(row["position"]), str(row["state"]),
                str(row["coord"]), str(row.get("player") or "—"),
                row.get("metal"), row.get("minerals"), row.get("gas"),
        ...
        with conn:
            conn.executemany(
                """INSERT INTO raid_queue(
                    imported_legacy_id, position, state, coord, player,
                    metal, minerals, gas, last_spy_at, enabled, blacklisted,
        ...
        with conn:
            ...
            conn.executemany(
                """INSERT OR IGNORE INTO recon_targets(
                    coord, player, enabled, blacklisted, notes,
                    created_at, updated_at
                ) VALUES(?,?,?,?,?,?,?)""",
                payload,
            )
        ...
        with conn:
            ...
            conn.executemany(
                """INSERT OR IGNORE INTO recon_reports(
                    report_id, target, report_at, energy, metal, minerals,
                    gas, source, ingested_at
                ) VALUES(?,?,?,?,?,?,?,?,?)""",
                [
                    (
                        str(row["report_id"]), str(row["target"]),
                        str(row["report_at"]), row.get("energy"),
                        row.get("metal"), row.get("minerals"), row.get("gas"),
                        str(row.get("source") or "messages"), now,
                    )
                    for row in rows
                ],
            )
    ...
    def list_recon_report_rows(self, *, limit: int = 2000) -> list[dict[str, object]]:
        rows = self._require_conn().execute(
            """SELECT id, report_id, target, report_at, energy,
                      metal, minerals, gas, source, ingested_at
                 FROM recon_reports
        ...
        self._require_conn().executescript(
            """CREATE TABLE IF NOT EXISTS raid_actions (
                ...
                status TEXT NOT NULL CHECK(
                    status IN ('pending','verified','ambiguous','rejected','failed')
                ),
                fleet_id TEXT,
                sent_at TEXT,
                arrival_at TEXT,
    ...
    def _migrate_to_4(self) -> None:
        self._require_conn().executescript(
            """CREATE TABLE IF NOT EXISTS spy_actions (
                ...
                status TEXT NOT NULL CHECK(
                    status IN ('pending','verified','ambiguous','failed_safe')
                ),
                report_id TEXT,
                requested_at TEXT,
                report_at TEXT,
    ...
    def _migrate_to_5(self) -> None:
        ...
        conn.execute(
            ...
            SELECT id, request_id, NULL, source, target, status, report_id,
                   ...
                   CASE
                     WHEN detail='' THEN
                       'migrated from V2 schema 4: fleet_id was not recorded'
                     ELSE detail ||
                       ' | migrated from V2 schema 4: fleet_id was not recorded'
                   END,
                   created_at, updated_at
    ...
    def _migrate_to_8(self) -> None:
        """Add immutable V2-owned asteroid observations for candidate state."""
        from v2.persistence.asteroid_candidates import install_asteroid_candidate_schema
