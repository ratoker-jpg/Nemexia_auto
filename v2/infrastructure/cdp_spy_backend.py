...
def select_verified_report(
    ...
    *,
    ...
    """Select only a new exact-target report fresh enough to verify this attempt."""
...
class V2SpyCdpBackend(ReadOnlyAccountCdpBackend):
    ...
    No browser launch, page creation, navigation, message deletion, CAPTCHA action,
    or retry is allowed. Verification comes only from a newly observed exact-target
    spy report after one invocation of the game's existing `processSpy` function.
    """
    ...
    async def _spy_row(self, fleet_id: str) -> dict[str, object]:
            ...
            row = await page.evaluate(
                r"""fleetId => {
                    ...
                    for (const tr of rows) {
                        const link=tr.querySelector(`#spy1Link-${CSS.escape(fleetId)}`)
        ...
        if not row:
            raise SpyActionError(
                f"Spy fleet {fleet_id} не найден среди уже загруженных espionage rows"
            )
    ...
    async def _reports_ready(self) -> tuple[object, tuple[SpyReportFact, ...]]:
        ...
        if not bool(raw.get("ready")):
            raise SpyActionError(
                "Открой системные сообщения на options.php до запуска разведки"
            )
    ...
    async def _request_spy(
        self,
        command: SpyRequestCommand,
        preparation: SpyRequestPreparation,
    ) -> SpyRequestResult:
        # Re-prove all facts after the pending journal row exists.
        row = await self._spy_row(command.fleet_id)
        ...
        before_ids = frozenset(
            str(item.report_id) for item in before_reports if item.report_id is not None
        )
        requested_at = datetime.now(timezone.utc)

        # Exactly one game mutation attempt. Never call this again in this method.
        try:
            await fleets_page.evaluate(
                """fleetId => {
        ...
        # `loadTabContent` refreshes the already-open report DOM; it does not navigate.
        # It is called once after the mutation solely to obtain verification evidence.
        try:
            await asyncio.sleep(0.35)
            if await self._captcha_present(fleets_page) or await self._captcha_present(options_page):
                raise SpyCaptchaBlocked(
                    "CAPTCHA появилась после processSpy; запрос считается неоднозначным"
                )
            await options_page.evaluate(
                """() => {
                    if (typeof window.loadTabContent !== 'function') {
                        throw new Error('loadTabContent missing');
                    }
        ...
        while time.monotonic() < deadline:
            ...
            if bool(raw.get("captcha_present")):
                raise SpyCaptchaBlocked(
                    "CAPTCHA появилась при проверке spy report; повтор запрещён"
                )
    ...
    def prepare(self, command: SpyRequestCommand) -> SpyRequestPreparation:
        try:
            return self._submit(self._prepare_spy(command), timeout=self.timeout_seconds + 2.0)
    ...
    def request(
        ...
        try:
            return self._submit(
                self._request_spy(command, preparation),
